...
joinedDF5 <- repartition(joinedDF5, 16) # write.df below will produce this many files

# write result to Parquet files
write.df(joinedDF5, file.path(fullDataDir, "joined5ParquetSubset"), "parquet", "overwrite")
...
joinedDF5Parquet <- RxParquetData(file.path(dataDir, "joined5ParquetSubset"),
                           colInfo = colInfo)

destData <- RxXdfData(file.path(dataDir, "joined5XDFSubset"))

rxImport(inData = joinedDF5Parquet, destData, overwrite = TRUE)
